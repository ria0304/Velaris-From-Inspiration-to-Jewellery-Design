...
The diagram above is the 30-second version. This is the full picture: every module, from the React entry point down through the FastAPI routes, the LLM design pipeline, the four ML models, and the offline training scripts that produce their checkpoints.

> Note: node labels link out to the source file on GitHub. That works when you view this diagram in an editor with Mermaid support (VS Code + a Mermaid extension, mermaid.live, etc.) — GitHub's own Markdown renderer strips `click` interactivity, so on github.com the nodes will render but won't be clickable.
