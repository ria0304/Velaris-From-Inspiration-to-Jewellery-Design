...
import type { DesignPackage, GemOption, AdvisorResponse, TrendReport } from './types';
...
export default function App() {
  ...
  const generateJewelryDesign = async () => {
    ...
    try {
      ...
      console.error(err);
      setErrorMsg(err instanceof Error ? err.message : "An unexpected error occurred during jewelry formulation.");
    } finally {
      setIsGenerating(false);
    }
  };
  ...
  const getGiftAdvice = async () => {
    ...
    try {
      ...
      setErrorMsg(err instanceof Error ? err.message : "Advisor recommendation model encountered an issue.");
    } finally {
  ...
  // Export saved design as jeweller-ready PDF via backend
  const exportPdf = async (designId: string, designName: string) => {
    try {
      ...
      link.download = data.filename || `${designName.replace(/\s+/g, "_")}.pdf`;
      link.click();
    } catch (err) {
  ...
  const deleteDesign = async (e: React.MouseEvent, designId: string) => {
    e.stopPropagation();
    try {
      const res = await fetch(`/api/saved-designs/${designId}`, { method: "DELETE" });
      if (res.ok) fetchSavedDesigns();
    } catch (err) {
  ...
  const copyAdvisorConcept = (rec: GemOption) => {
    const words = rec.explanation.split(" ").slice(0, 20).join(" ");
    setInputType('text');
    setTextPrompt(`A modern ${rec.gemstone} piece suited for an elegant ${advisorOccasion}. Style guideline: ${words}...`);
    setActiveTab('design');
                ...
                <div className="mt-6 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-3 border-t border-[#122e1e]/60">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <select
                        value={budgetTier}
                        onChange={(e) => setBudgetTier(e.target.value as 'Value' | 'Balanced' | 'Premium')}
                        className="w-full text-xs p-2.5 bg-[#020503] border border-[#122e1e] rounded text-gray-200 focus:outline-none focus:border-[#d4af37]"
            ...
            <section className="lg:col-span-7 space-y-8">
              {currentDesign ? (
                <div className="space-y-6 print-full" id="active-design-package-content">
                  ...
                  <PresentationBoard design={currentDesign} />
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                          ...
                          <div>
                            ...
                            <div className="w-full bg-[#030906] h-1.5 overflow-hidden rounded-full border border-[#1a3a29]/30">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto" id="saved-designs-collection-grid">
                {savedDesigns.map((design, index) => (
                  ...
                  >
                    ...
                    <div className="p-4 bg-[#0a1410] border-t border-[#122e1e]/60 flex gap-2">
                      <button
                        onClick={(e) => { e.stopPropagation(); exportPdf(design.id, design.name); }}
                        className="flex-1 text-xs text-black bg-[#d4af37] font-semibold font-mono tracking-widest text-center uppercase hover:bg-[#ebd582] transition rounded py-2"
                      >
                        Export PDF
                      </button>
                      <button
                        onClick={(e) => deleteDesign(e, design.id)}
                        className="px-3 text-xs text-red-300 border border-red-900/50 font-mono uppercase rounded py-2 hover:bg-red-950/40 transition"
                      >
                        Delete
                      </button>
                    </div>
