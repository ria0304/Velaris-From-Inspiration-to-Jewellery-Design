...
def _load_captions() -> dict:
    ...
    # This dataset revision ships captions as dataset/dataset_labels.csv
    # with columns image_path,description (bass names may carry backslashes).
    csv_path = RAW_DIR / "dataset" / "dataset_labels.csv"
    if csv_path.exists():
        out = {}
        with open(csv_path, newline="", encoding="utf-8", errors="replace") as f:
            for row in csv.DictReader(f):
                name = (row.get("image_path") or "").replace("\\", "/").split("/")[-1]
                if name:
                    out[name] = row.get("description", "")
        if out:
            return out
