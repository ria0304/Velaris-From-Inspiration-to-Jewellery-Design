...
def _extract_svg(raw: str) -> Optional[str]:
    """Pull the first complete <svg…</svg> block out, validate + sanitize."""
    start = raw.find("<svg")
    end   = raw.rfind("</svg>")
    if start == -1 or end == -1:
        return None
...
def sanitize_svg(svg: str | None) -> Optional[str]:
    """Strip scripts/event-handlers/javascript:, enforce viewBox + min elements."""
    if not svg or "<svg" not in svg:
        return None
    # remove script tags + event handler attributes + javascript: urls
    svg = re.sub(r"<script.*?</script>", "", svg, flags=re.DOTALL | re.IGNORECASE)
    svg = re.sub(r"\son\w+\s*=\s*(\"[^\"]*\"|'[^']*'|[^\s>]+)", "", svg, flags=re.IGNORECASE)
    ...
    if not re.search(r"<(path|rect|circle|ellipse|polygon|polyline|line|g)\b", svg):
        return None
    # enforce exact viewBox 0 0 200 200
    if "viewBox" in svg:
        svg = re.sub(r'viewBox\s*=\s*"[^"]*"', 'viewBox="0 0 200 200"', svg)
    else:
        svg = svg.replace("<svg", '<svg viewBox="0 0 200 200"', 1)
    # count distinct elements — premium illustration floor
    if len(re.findall(r"<(path|rect|circle|ellipse|polygon|polyline|line)\b", svg)) < 6:
        return None
    return svg


def scope_ids(svg: str, uid: str) -> str:
    """Namespace gradient/filter ids per view to avoid collisions when cached."""
    for old in ("gemGlow", "metalSheen", "metal", "gem", "soft"):
        svg = svg.replace(f'id="{old}"', f'id="{uid}-{old}"')
        svg = re.sub(r"url\(#" + old + r"\)", f"url(#{uid}-{old})", svg)
    return svg
...
def generate_svg_endpoint(req: SVGRequest) -> SVGResponse:
    # Primary: deterministic parametric CAD — consistent across all 3 views,
    # true to spec (carat/shape/band/setting), manufacturable + sanitized.
    try:
        from .cad import render_cad_svg
        svg, _ = render_cad_svg(
            req.jewelry_type, req.metal, req.stone, req.stone_shape,
            req.stone_size, req.setting, None, req.view, req.design_id)
        return SVGResponse(svg=svg, model_used="parametric-cad")
    except Exception as e:
        print(f"Parametric CAD failed ({e}), trying LLM artistic + fallback")
    try:
        ...
        if not svg:
            ...
            svg = scope_ids(svg or "", f"{req.design_id}-{req.view}")
            model_used = "fallback"

        svg = scope_ids(svg, f"{req.design_id}-{req.view}")
