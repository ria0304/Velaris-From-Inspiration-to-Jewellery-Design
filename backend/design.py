...
# ─── NEW: Module 3 — YOLO gemstone detector ───────────────────────────────────
...
# the image alone. Same fail-soft pattern as Module 1 — no weights, no
# ultralytics, no problem, this just becomes a no-op.
try:
    from .ml.gemstone_detector import detect_gemstones, CONFIDENCE_THRESHOLD as GEMSTONE_CONFIDENCE_THRESHOLD
    GEMSTONE_DETECTOR_AVAILABLE = True
except ImportError:
    GEMSTONE_DETECTOR_AVAILABLE = False
    GEMSTONE_CONFIDENCE_THRESHOLD = 0.35
...
def resolve_gemstone_hint(req: DesignRequest) -> Optional[str]:
    ...
    confident_labels = sorted(
        {d["label"] for d in detections if d["confidence"] >= GEMSTONE_CONFIDENCE_THRESHOLD}
    )
