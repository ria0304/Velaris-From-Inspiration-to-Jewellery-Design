...
# ─── NEW: Module 1 — CNN type classifier ──────────────────────────────────────
# When a sketch/photo comes in, we run it through the trained EfficientNet
# classifier BEFORE calling the LLM, instead of letting the LLM guess the
# jewelry type blind from the image. If the classifier isn't trained yet
# (no checkpoint) or torch isn't installed, this fails soft and design.py
# behaves exactly as before — text-keyword extraction only.
try:
    from .ml.type_classifier import classify_type_from_image, CONFIDENCE_THRESHOLD
    TYPE_CLASSIFIER_AVAILABLE = True
except ImportError:
    TYPE_CLASSIFIER_AVAILABLE = False
    CONFIDENCE_THRESHOLD = 0.55

# The classifier is only trained on these four classes (the ones the public
# dataset actually has labels for). If the user's text explicitly names one
# of these, there's no point asking a 4-class model to weigh in.
_UNCOVERED_TYPE_KEYWORDS = ("brooch", "pendant", "tiara", "crown")

# ─── NEW: Module 3 — gemstone detector (hybrid YOLO / OWL-ViT backends) ───────
# Optional enrichment: if gemstones are detected in an uploaded photo/sketch,
# pass them to the LLM as a hint rather than letting it guess stones from
# the image alone. Same fail-soft pattern as Module 1 — no backends
# available, no problem, this just becomes a no-op. The threshold is
# backend-aware (strict for finetuned YOLO, lenient for zero-shot OWL-ViT).
try:
    from .ml.gemstone_detector import detect_gemstones, effective_threshold
    GEMSTONE_DETECTOR_AVAILABLE = True

    def _gemstone_threshold() -> float:
        return effective_threshold()
except ImportError:
    GEMSTONE_DETECTOR_AVAILABLE = False

    def _gemstone_threshold() -> float:
        return 0.35


def resolve_gemstone_hint(req: DesignRequest) -> Optional[str]:
    """
    Runs Module 3 on a sketch/photo upload and turns detections into a short
    hint string for the prompt, e.g. "Diamond, Ruby". Returns None if the
    detector isn't available, there's no image, or nothing was detected
    above threshold — never raises, this is a pure enrichment step.
    """
    if not (GEMSTONE_DETECTOR_AVAILABLE and req.image and req.inputType in ("photo", "sketch")):
        return None

    detections = detect_gemstones(req.image)
    if not detections:
        return None

    confident_labels = sorted(
        {d["label"] for d in detections if d["confidence"] >= _gemstone_threshold()}
    )
    return ", ".join(confident_labels) if confident_labels else None
...
def extract_design_type(prompt: str) -> str:
    ...
    if "brooch" in prompt_lower or "brooches" in prompt_lower:
        return "Brooch"
    elif "necklace" in prompt_lower or "necklaces" in prompt_lower or "chain" in prompt_lower:
        return "Necklace"
    elif "bracelet" in prompt_lower or "bracelets" in prompt_lower or "bangle" in prompt_lower:
        return "Bracelet"
    elif "earring" in prompt_lower or "earrings" in prompt_lower or "ear ring" in prompt_lower or "stud" in prompt_lower:
        return "Earrings"
    elif "pendant" in prompt_lower or "pendants" in prompt_lower:
        return "Pendant"
    elif "tiara" in prompt_lower or "tiaras" in prompt_lower or "crown" in prompt_lower:
        return "Tiara"
    elif "ring" in prompt_lower or "rings" in prompt_lower:
        return "Ring"
    else:
...
def resolve_design_type(req: DesignRequest) -> Tuple[str, Optional[Dict]]:
    """
    Decides the expected jewelry type using text extraction, with the
    Module 1 CNN classifier as a stronger signal when a sketch/photo is
    present and the text doesn't already name an uncovered type
    (Brooch/Pendant/Tiara, which the classifier was never trained on).

    Returns (design_type, classification_result_or_None). The classification
    dict (when present) also carries per-class confidence, useful for logging
    and for the /api/classify-type demo endpoint's evaluation story.
    """
    text_type = extract_design_type(req.prompt)

    prompt_lower = (req.prompt or "").lower()
    explicit_uncovered = any(kw in prompt_lower for kw in _UNCOVERED_TYPE_KEYWORDS)

    if (
        TYPE_CLASSIFIER_AVAILABLE
        and req.image
        and req.inputType in ("photo", "sketch")
        and not explicit_uncovered
    ):
        classification = classify_type_from_image(req.image)
        if classification and classification["confidence"] >= CONFIDENCE_THRESHOLD:
            print(
                f"[type_classifier] predicted {classification['type']} "
                f"({classification['confidence']:.0%} confidence) — using CNN over text extraction"
            )
            return classification["type"], classification
        return text_type, classification

    return text_type, None


def _build_prompt(req: DesignRequest, design_type: str, gemstone_hint: Optional[str] = None) -> str:
    """
    Build a prompt that strongly enforces the design type.
    `design_type` is resolved once in generate_design() via
    resolve_design_type() so the LLM prompt and the post-hoc validation
    always agree on the same expected type. `gemstone_hint` comes from
    Module 3 (YOLO) when it detected stones in an uploaded image with
    reasonable confidence — None if unavailable/nothing detected.
    """
    ...
    if gemstone_hint:
        type_emphasis += f"""
    💎 DETECTED GEMSTONES: Automated gemstone detection found the following stone(s)
    in the uploaded image: {gemstone_hint}. Prefer these unless the user's text explicitly
    requests different stones.
    """
...
def generate_design(req: DesignRequest) -> FinalDesignPackage:
    expected_type, classification_result = resolve_design_type(req)
    gemstone_hint = resolve_gemstone_hint(req)
    prompt_str = _build_prompt(req, expected_type, gemstone_hint)
    ...
    # (expected_type was already resolved above — CNN classification for
    # sketch/photo inputs when confident, text extraction otherwise)
    ...
    return FinalDesignPackage(
        ...
        spec=DesignSpec(
            ...
            bandWidth=raw_data.bandWidth or ("1.8 mm" if raw_data.type == "Ring" else "N/A"),
        ...
        modelUsed=(
            f"{model_used} (+ CNN type classifier, {classification_result['confidence']:.0%} conf.)"
            if classification_result and classification_result["confidence"] >= CONFIDENCE_THRESHOLD
            else model_used
        ),
    )
